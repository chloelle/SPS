...
#pragma omp parallel
	  {
#pragma omp for
		for (vl_index y = 1 ; y < (signed)height-1 ; ++y) {
		  for (vl_index x = 1 ; x < (signed)width-1 ; ++x) {
...
#pragma omp parallel
  {
#pragma omp for
	  for (vl_index v = 0 ; v < (signed)numRegionsY ; v++) {
		for (vl_index u = 0 ; u < (signed)numRegionsX ; u++) {
		  vl_index xp ;
		  vl_index yp ;
		  vl_index centerx = 0 ;
		  vl_index centery = 0 ;
		  ...
		  vl_index tx = (vl_index) vl_round_d(regionSize * (u + 0.5)) ;
		  vl_index ty = (vl_index) vl_round_d(regionSize * (v + 0.5)) ;

		  tx = VL_MAX(VL_MIN(tx, (signed)width-1),0) ;
		  ty = VL_MAX(VL_MIN(ty, (signed)height-1),0) ;

		  /* search in a 3x3 neighbourhood the smallest edge response */
		  for (yp = VL_MAX(0, ty-1) ; yp <= VL_MIN((signed)height-1, ty+1) ; ++ yp) {
			for (xp = VL_MAX(0, tx-1) ; xp <= VL_MIN((signed)width-1, tx+1) ; ++ xp) {
		  ...
		  centers[v*(signed)numRegionsX*(2+numChannels)+u*(2+numChannels)] = (float) centerx ;
		  centers[v*(signed)numRegionsX*(2+numChannels)+u*(2+numChannels)+1] = (float) centery ;
		  for (k  = 0 ; k < (signed)numChannels ; ++k) {
			//centers[i++] = atimage(centerx,centery,k) ;
			centers[v*(signed)numRegionsX*(2+numChannels)+u*(2+numChannels)+(2+k)] = atimage(centerx,centery,k) ;
		  }
		}
	  }
  ...
  /* run k-means iterations */
	  ...
	  for (iter = 0 ; iter < maxNumIterations ; ++iter) {
		...
		float energy = 0 ;
			...
			for (vl_index y = 0 ; y < (signed)height ; y++) {

			  for (vl_index x = 0 ; x < (signed)width ; x++) {
...
#pragma omp parallel
		{
#pragma omp for
			for (vl_index region = 0 ; region < (signed)numRegions ; ++region) {
  ...
  /* elimiate small regions */
  {
    ...
    vl_index direction ;
		...
		for (vl_index pixel = 0 ; pixel < (signed)numPixels ; pixel++) {
		  if (!cleaned[pixel]){
			  ...
			  vl_index x = pixel % width ;
			  vl_index y = pixel / width ;
